  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, position: 'relative', overflow: 'hidden' }}>
      ...
      >
        ...
        <h1 style={{ fontSize: 26, fontWeight: 700, color: 'white', marginBottom: 6 }}>Welcome Back</h1>
        <p style={{ fontSize: 13, color: '#6b7280', marginBottom: 24 }}>Sign in to continue your journey.</p>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              <input type={showPw ? 'text' : 'password'} placeholder="••••••••" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required style={{ ...inp, paddingRight: 44 }}
                onFocus={(e) => e.target.style.borderColor = '#3b82f6'} onBlur={(e) => e.target.style.borderColor = 'rgba(255,255,255,0.1)'} />
        ...
        <p style={{ textAlign: 'center', fontSize: 13, color: '#6b7280', marginTop: 20 }}>
          Don't have an account? <Link to="/signup" style={{ color: '#60a5fa', textDecoration: 'none', fontWeight: 500 }}>Create one</Link>
        </p>
      </motion.div>
    </div>
  )
